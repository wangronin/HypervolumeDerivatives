...
pareto_front = np.array([F(_) for _ in pareto_set])
# ref = pareto_front - np.array([0.01, 0.01])
...
ax0.plot(X[:, 0], X[:, 1], "g*")
ax0.plot(x0[:, 0], x0[:, 1], "r+", ms=8, clip_on=False)
ax0.set_xlim([-2, 2])
ax0.set_ylim([-2, 2])
...
ax1.plot(Y[:, 0], Y[:, 1], "g*")
ax1.plot(y0[:, 0], y0[:, 1], "r+", ms=8)
ax1.plot(pareto_front[:, 0], pareto_front[:, 1], "k.", ms=8)
