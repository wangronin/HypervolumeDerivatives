...
ref = pd.read_csv("./data/CONV3_linear_reference.csv", header=None).values
x0 = pd.read_csv("./data/CONV3_X0_line_no_rep.csv", header=None).values
...
lines += ax.plot(reference_set[:, 0], reference_set[:, 1], reference_set[:, 2], "g+", ms=13)
lines.append(ax.plot_trisurf(triang, z, color="k", alpha=0.2))
ax.legend(lines, [r"$Y_{\text{final}}$", "reference set", r"Pareto front"], loc="lower right")
ax.set_title("objective space")
ax.set_xlabel(r"$f_1$")
ax.set_ylabel(r"$f_2$")
ax.set_zlabel(r"$f_3$")
...
plt.savefig(f"CONV3-DpN-degenerate-example-{N}.pdf", dpi=100)
